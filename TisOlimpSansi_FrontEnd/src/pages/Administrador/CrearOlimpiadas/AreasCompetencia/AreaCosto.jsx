...
const AreaCosto = ({ area, actualizarCosto }) => {
  ...
  const getAreaImage = (areaName) => {
    ...
    if (normalizedName.includes("MATEMATICA"))
      return "/images/matematicaas.png";
    ...
    if (normalizedName.includes("INFORMATICA"))
      return "/images/informatica.png";
          ...
          {(area.niveles || area.rangos) && (
            <div className="mt-2">
              <p className="text-sm text-gray-600">
                {area.niveles
                  ? `${area.niveles.length} ${
                      area.niveles.length === 1 ? "nivel" : "niveles"
                    } configurados`
                  : area.rangos
                  ? `${area.rangos.length} ${
                      area.rangos.length === 1 ? "nivel" : "niveles"
                    } configurados`
          ...
          <div className="mt-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <button
                ...
                className="bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full border border-blue-200 hover:bg-blue-100 text-sm flex items-center justify-center gap-1"
              >
                <FaCoins className="text-sm" />
                {area.costoInscripcion && Number(area.costoInscripcion) > 0
                  ? "Modificar costo"
                  : "Asignar costo"}
              </button>
            </div>
