...
export const normalizarDatosEstudiante = (originalData) => {
        ...
        if (area.nombre_area) {
          const matchedArea = areas.find(
            (a) =>
              ...
              normalizeForComparison(area.nombre_area)
          );
          if (matchedArea) {
  ...
  if (normalizedData.colegio?.curso) {
    ...
    const cursoNormalizado = cursos.find((c) => {
      const simplifiedCurso = normalizeForComparison(cursoOriginal).replace(
        /\s+de\s+/i,
        " "
      );
      const simplifiedOption = normalizeForComparison(c).replace(
        /\s+de\s+/i,
        " "
      );
      return (
        simplifiedCurso.includes(simplifiedOption) ||
        simplifiedOption.includes(simplifiedCurso)
      );
    });
  ...
  if (normalizedData.colegio?.departamento) {
    ...
    const deptoNormalizado = Object.keys(departamentos).find(
      (d) => normalizeForComparison(d) === normalizeForComparison(deptoOriginal)
    );
    ...
    if (deptoNormalizado) {
      ...
      if (normalizedData.colegio?.provincia) {
        ...
        const provinciaNormalizada = departamentos[deptoNormalizado].find(
          (p) =>
            normalizeForComparison(p) ===
            normalizeForComparison(provinciaOriginal)
        );
  ...
  if (normalizedData.tutores_academicos) {
    normalizedData.tutores_academicos = normalizedData.tutores_academicos.map(
      (tutor) => {
        if (tutor.nombre_area) {
          const matchedArea = areas.find(
            (a) =>
              normalizeForComparison(a) ===
              normalizeForComparison(tutor.nombre_area)
          );
          if (matchedArea) {
